...
{
    /// <summary>
    /// Helper Class for serializing and deserializing Opaque cursors from Integer Indexed based result sets.
    /// </summary>
    public static class CursorFactory
    {
        ...
        public static int ParseCursor(string cursor)
        {
            ...
            var bytesEncoded = Utf8.GetBytes(cursor.AsSpan(), bufferSpan);

            // Decode only the bytes written above so ParseCursor mirrors the exact cursor data provided.
            Base64.DecodeFromUtf8InPlace(bufferSpan.Slice(0, bytesEncoded), out var bytesWritten);
